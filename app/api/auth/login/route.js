...
export async function POST(req) {
  ...
  const users = await db`SELECT * FROM app_users WHERE username = ${username} LIMIT 1`
  ...
  if (users.length === 0) {
    ...
    if (noUserYet && username === 'user' && password === '1234') {
      ...
      `
      await db`INSERT INTO user_profiles (user_id) VALUES (${created[0].id}) ON CONFLICT DO NOTHING`
